...
function regimentoIndexarArtigos() {
  ...
  _regimentoArtigos = partes.map(parte => {
    const match = parte.match(/^Art\.?\s*(\d+[º°]?\s*[-–]?\s*[A-Z]?)/i);
    const num = match ? match[1].trim().replace(/[º°]/g,'') : '';
...
function regimentoRenderFontes(text) {
  ...
  for (const m of matches) { const ref = 'Art. ' + m[1]; if (!artigos.includes(ref)) artigos.push(ref); }
...
function regimentoNavigateToArt(artRef) {
  ...
  let capitulo = '';
  if (_regimentoArtigos.length > 0) {
    const artigo = _regimentoArtigos.find(a => a.num === artNum || a.num.replace(/\s/g,'') === artNum.replace(/\s/g,''));
    if (artigo) textoArtigo = artigo.texto;
  }
  // Se não encontrou no índice, buscar no texto bruto
  if (!textoArtigo && _regimentoTexto) {
    const regex = new RegExp('Art\\.?\\s*' + artNum.replace('-', '\\s*[-–]\\s*') + '[^A]*?(?=Art\\.?\\s*\\d|$)', 'i');
    const match = _regimentoTexto.match(regex);
    if (match) textoArtigo = match[0].trim();
  }
  if (!textoArtigo) textoArtigo = 'Texto do artigo nao encontrado na extracao do PDF.';
